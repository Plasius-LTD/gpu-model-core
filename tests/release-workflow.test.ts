...
describe("release snapshot selection", () => {
  it("binds publication to the immutable workflow-dispatch commit", () => {
    ...
    expect(assertReleaseSnapshot({
      dispatchSha,
      baseSha: dispatchSha,
      packageVersion: "0.2.0",
      changelog: "## [0.2.0] - 2026-07-15\n",
      targetVersion: "0.2.0",
    })).toBe(dispatchSha);
  });
  ...
  it("routes the workflow through the executable selector", async () => {
  ...
  it("rejects a moved remote ref while retaining immutable Git-object reads", async () => {
    const repository = await mkdtemp(join(tmpdir(), "gpu-model-core-release-"));
    ...
    await writeFile(join(repository, "CHANGELOG.md"), "## [0.2.0] - 2026-07-15\n");
    ...
    expect(execFileSync(process.execPath, [script.pathname], {
      cwd: repository,
      encoding: "utf8",
      env: environment,
    })).toBe(dispatchSha);
